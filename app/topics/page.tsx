  ...
  return (
    <div className="max-w-5xl mx-auto space-y-7 pb-10 anim-up">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-[14px]">Manage assessments and track student performance.</p>
          </div>
        </div>
        {/* Search */}
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="input-base pl-9 text-[13px] h-9"
            value={search}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 anim-up-1">
        {[
          { label: 'Total exams', value: history.length, icon: BarChart3, color: 'oklch(0.52 0.22 264)', bg: 'oklch(0.93 0.04 262)' },
          {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr style={{ background: 'oklch(0.97 0.005 255)' }}>
                {['Assessment', 'Created', 'Students', 'Avg. score', ''].map((h) => (
                  <th key={h} className="px-6 py-3 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
                ...
                <tr key={exam.id} className="trow">
                  ...
                  <td className="px-6 py-4">
                    <div className="space-y-1.5">
                      <span
                        ...
                        style={{ color: exam.averageScore >= 60 ? 'oklch(0.50 0.14 155)' : 'oklch(0.62 0.18 55)' }}
                      >
                        {exam.averageScore}%
                      </span>
                      <div className="w-20 h-1.5 rounded-full overflow-hidden" style={{ background: 'oklch(0.93 0.01 255)' }}>
                        <div
                          className="h-full rounded-full"
                          style={{
                            width: `${exam.averageScore}%`,
                            background: exam.averageScore >= 60 ? 'oklch(0.50 0.14 155)' : 'oklch(0.62 0.18 55)',
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link href={`/exams/${exam.id}/attempts`}>
                      <Button variant="ghost" size="sm" className="text-[12px] font-semibold gap-1 h-7 px-3 rounded-lg text-muted-foreground hover:text-foreground">
          ...
          {filteredHistory.length === 0 && (
            <div className="py-16 text-center text-muted-foreground text-[14px]">
              {search ? `No exams matching "${search}".` : 'No exam history found.'}
            </div>
          )}
        </div>
      </div>
